"""
Polymarket Top-of-Book Market Maker - CLOB Client
...
class PolymarketClient:
    ...
    - Order placement (limit GTC, market FOK)
    - Order cancellation
    - Position queries
    ...
    async def place_limit_order(
        ...
        """
        Place a limit order (GTC or IOC).
        ...
        Args:
            ...
            time_in_force: GTC (Good-Til-Cancelled), IOC (Immediate-Or-Cancel)
        ...
        # Force exact 2 decimal precision using Decimal for precise rounding
        from decimal import Decimal, ROUND_HALF_UP
        clean_price = float(Decimal(str(price)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
        clean_size = float(Decimal(str(size)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
        ...
        # Choose order type: GTC for passive bids, FAK/FOK for immediate execution
        # FAK = Fill-and-Kill (partial fills allowed)
        # FOK = Fill-or-Kill (all or nothing)
        if time_in_force == "GTC":
            order_type = OrderType.GTC
        elif time_in_force == "IOC" or time_in_force == "FAK":
    ...
    async def place_market_order(
        ...
        Args:
            ...
            use_ioc: If True, use IOC (partial fills allowed). If False, use FOK (all or nothing).
        """
        if not self._clob:
            raise RuntimeError("Client not initialized")
        ...
        # FAK = Fill-and-Kill (partial fills allowed, like IOC)
        # FOK = Fill-or-Kill (all or nothing)
        order_type = OrderType.FAK if use_ioc else OrderType.FOK
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        Returns:
            ...
            False if order was already matched/filled or cancel failed
        """
        if not self._clob:
            raise RuntimeError("Client not initialized")
            ...
            if resp.get("success") or resp.get("canceled"):
                ...
                if not_canceled:
                    ...
                    if "already matched" in reason:
                        # Order was filled - return False so caller knows not to place new order
                        logger.debug(f"Order already matched: {order_id[:16]}...")
                        return False
                    if "already canceled" in reason:
                        # Order was already cancelled - safe to place new order
                        logger.debug(f"Order already cancelled: {order_id[:16]}...")
                        return True
