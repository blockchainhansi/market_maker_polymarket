"""
Polymarket Top-of-Book Market Maker - Logging Configuration
...
class StrategyLogger:
    ...
    def bid_prices(self, yes_price: Optional[float], no_price: Optional[float]):
        """Log calculated bid prices."""
        yes_str = f"${yes_price:.4f}" if yes_price else "NO_QUOTE"
        no_str = f"${no_price:.4f}" if no_price else "NO_QUOTE"
        self.logger.debug(f"Bid prices: YES={yes_str}, NO={no_str}")
